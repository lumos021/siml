...
The Tier-1 reference watermark (`siml_watermark.py`) is measured at ~39 dB PSNR (visually negligible) and recovers a phone number cleanly through JPEG q30+, WebP, double-compression, screenshots, and downscale-to-900 + recompress. Below ~800px + recompress the signal is genuinely gone - so it **fails loudly** (a CRC rejects bad reads) rather than returning a wrong number, and hands off to T2.
