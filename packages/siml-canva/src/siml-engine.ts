...
const TEXTURE_EMBED_STDDEV = 8,
  MIN_TEXTURED_REPS = 8,
  MIN_POSITION_VOTES = 4,
  Q_SMOOTH = 13;
const PH_SIZE = 32,
  PH_LOW = 16;
const SIML_UUID = [
  0xde, 0x60, 0x62, 0xe1, 0x9e, 0x7b, 0x49, 0x4e, 0x94, 0xb3, 0xdb, 0x6f, 0xcc,
  0x25, 0x7f, 0xdc,
];
...
const rsMul = (a: number, b: number) =>
  a && b ? RS_EXP[RS_LOG[a] + RS_LOG[b]] : 0;

function rsEncode(data: number[], nsym: number): number[] {
  let g = [1];
  for (let i = 0; i < nsym; i++) {
    const q = [1, RS_EXP[i]],
      r = new Array(g.length + 1).fill(0);
    for (let a = 0; a < g.length; a++)
      for (let b = 0; b < 2; b++) r[a + b] ^= rsMul(g[a], q[b]);
    g = r;
  }
...
for (let u = 0; u < 8; u++)
  for (let x = 0; x < 8; x++)
    cosT[u * 8 + x] = Math.cos(((2 * x + 1) * u * Math.PI) / 16);
...
function dct2d(blk: Float32Array): Float32Array {
  ...
  for (let u = 0; u < 8; u++) {
    ...
    for (let v = 0; v < 8; v++) {
      ...
      for (let x = 0; x < 8; x++) {
        const cx = cosT[u * 8 + x];
        for (let y = 0; y < 8; y++) s += blk[x * 8 + y] * cx * cosT[v * 8 + y];
      }
    ...
    for (let y = 0; y < 8; y++) {
      ...
      for (let u = 0; u < 8; u++) {
        const cu = cc(u),
          cx = cosT[u * 8 + x];
        for (let v = 0; v < 8; v++)
          s += cu * cc(v) * d[u * 8 + v] * cx * cosT[v * 8 + y];
      }
      out[x * 8 + y] = 0.25 * s;
    }
  return out;
...
export function embedWatermark(
  ...
  const rsCoded = rsEncode(
    [...Array.from(payloadBytes), (crc >> 8) & 0xff, crc & 0xff],
    RS_NSYM,
  );
  const bits: number[] = [];
  ...
  for (let by = 0; by < bY; by++)
    for (let bx = 0; bx < bX; bx++) {
      ...
      for (let x = 0; x < 8; x++)
        for (let y = 0; y < 8; y++)
          m += Y0[(by * 8 + x) * width + (bx * 8 + y)];
      m /= 64;
      let v = 0;
      for (let x = 0; x < 8; x++)
        for (let y = 0; y < 8; y++) {
          const d = Y0[(by * 8 + x) * width + (bx * 8 + y)] - m;
          v += d * d;
        }
      if (Math.sqrt(v / 64) >= TEXTURE_EMBED_STDDEV) {
        mask[by * bX + bx] = 1;
        eligible++;
      }
    }
  ...
  for (let i = 0; i < mask.length; i++)
    if (mask[i]) posVotes[i % bits.length]++;
  let minVotes = Infinity;
  for (let i = 0; i < bits.length; i++)
    if (posVotes[i] < minVotes) minVotes = posVotes[i];
  const selective =
    eligible >= MIN_TEXTURED_REPS * bits.length &&
    minVotes >= MIN_POSITION_VOTES;
  if (!selective)
    for (let i = 0; i < mask.length; i++) if (!mask[i]) mask[i] = 2; // dual-band
  ...
  for (let pass = 0; pass < 3; pass++) {
    ...
    for (let by = 0; by < bY; by++)
      for (let bx = 0; bx < bX; bx++) {
        ...
        for (let x = 0; x < 8; x++)
          for (let y = 0; y < 8; y++)
            blk[x * 8 + y] = Y[(by * 8 + x) * width + (bx * 8 + y)];
        const d = dct2d(blk),
          target = bits[bi % bits.length],
          c = d[2 * 8 + 1];
        ...
        if (mode === 2) d[2 * 8 + 1] = level;
          ...
          d[2 * 8 + 1] = level + Math.max(-band, Math.min(band, c - level));
        }
        const rec = idct2d(d);
        for (let x = 0; x < 8; x++)
          for (let y = 0; y < 8; y++)
            Y[(by * 8 + x) * width + (bx * 8 + y)] = rec[x * 8 + y];
      }
    for (let i = 0; i < width * height; i++) {
      const r = rgba[i * 4],
        g = rgba[i * 4 + 1],
        b = rgba[i * 4 + 2];
      const oy = 0.299 * r + 0.587 * g + 0.114 * b,
        dl = Y[i] - oy;
...
export function selectT1Payload(
  layer: LayerObject[],
  contentId: string,
): { value: string; mode: string } | null {
  const fits = (s: string) => new TextEncoder().encode(s).length <= T1_CAPACITY;
  const chosen =
    layer.find((o) => o.primary === true) ||
    layer.find(
      (o) =>
        (o.intent || "actionable") === "actionable" && ACTIONABLE.has(o.type),
    );
...
const phB = (() => {
  ...
  for (let u = 0; u < PH_SIZE; u++)
    for (let x = 0; x < PH_SIZE; x++)
      b[u * PH_SIZE + x] = Math.cos(
        ((2 * x + 1) * u * Math.PI) / (2 * PH_SIZE),
      );
  return b;
})();
...
function pHashGrey(grey: Float64Array): string {
  ...
  for (let y = 0; y < PH_SIZE; y++)
    for (let u = 0; u < PH_SIZE; u++) {
      let s = 0;
      for (let x = 0; x < PH_SIZE; x++)
        s += grey[y * PH_SIZE + x] * phB[u * PH_SIZE + x];
      rows[y * PH_SIZE + u] = s;
    }
  ...
  for (let u = 0; u < PH_SIZE; u++)
    for (let v = 0; v < PH_SIZE; v++) {
      let s = 0;
      for (let y = 0; y < PH_SIZE; y++)
        s += rows[y * PH_SIZE + v] * phB[u * PH_SIZE + y];
      d[u * PH_SIZE + v] = s;
    }
  const co = new Float64Array(PH_LOW * PH_LOW);
  let k = 0;
  for (let u = 0; u < PH_LOW; u++)
    for (let v = 0; v < PH_LOW; v++) co[k++] = d[u * PH_SIZE + v];
  const sorted = Array.from(co)
    .slice(1)
    .sort((a, b) => a - b);
  const med = sorted[Math.floor(sorted.length / 2)];
  const h = new Uint8Array(32);
  for (let i = 0; i < PH_LOW * PH_LOW; i++)
    if (co[i] > med) h[i >> 3] |= 1 << (7 - (i & 7));
  return Array.from(h)
...
export function regionHashOf(
  ...
  let left = Math.round(((b.x - padX) / 100) * W),
    top = Math.round(((b.y - padY) / 100) * H);
  let cw = Math.round(((b.w + 2 * padX) / 100) * W),
    ch = Math.round(((b.h + 2 * padY) / 100) * H);
...
export function injectPNG(buf: Uint8Array, payload: Uint8Array): Uint8Array {
  ...
  while (pos < buf.length) {
    ...
    if (type === "IEND")
      return concat([buf.subarray(0, pos), chunk, buf.subarray(pos)]);
    pos += 12 + len;
  }
  return concat([buf, chunk]);
}

// T0 JUMBF injection for JPEG (APP11 segment, "JP02" framing - matches the
// canonical embed-jpeg.js). Inserts before the SOS marker.
export function injectJPEG(buf: Uint8Array, jumbf: Uint8Array): Uint8Array {
  if (buf[0] !== 0xff || buf[1] !== 0xd8) throw new Error("not a valid JPEG");
  const segLen = 2 + 4 + 4 + jumbf.length; // size(2) + "JP02"(4) + index(4) + payload
  if (segLen > 0xffff) throw new Error("SIML payload too large for one JPEG APP11 segment");
  const header = new Uint8Array(12);
  header[0] = 0xff; header[1] = 0xeb; // APP11
  header[2] = (segLen >>> 8) & 0xff; header[3] = segLen & 0xff;
  header[4] = 0x4a; header[5] = 0x50; header[6] = 0x30; header[7] = 0x32; // "JP02"
  header[8] = 0; header[9] = 0; header[10] = 0; header[11] = 1; // segment index 1
  ...
  while (pos < buf.length - 1) {
    ...
    if (marker === 0xffda) { insertAt = pos; break } // SOS
